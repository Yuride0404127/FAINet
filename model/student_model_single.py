...
# from KD_model_3.Conv4d import Conv4d
...
class ACA_v3(nn.Module):
    def __init__(self, in_channel, out_channel, hw):
        ...
        self.conv_compress = DSConv3x3(2 * out_channel, out_channel)
    ...
    def forward(self, x_up, x_down):
        if (self.in_channel == self.out_channel):
            x_down = self.conv_inter(x_down)
        else:
            x_down = self.pool2(self.conv_inter(x_down))
        ...
        x_cat = torch.cat([x_add, (x_up + x_down)], dim=1)
...
class FDW_v2(nn.Module):
    ...
    def forward(self, x_up, x_down):
        ...
        out = self.conv_out(Muti_up + Muti_down) + x_up

        return out


class KD_model_3_student(nn.Module):
    def __init__(self):
        super(KD_model_3_student, self).__init__()
        # Backbone model
    ...
    def forward(self, x_rgb):
        ...
        aca1_out = self.aca1(rgb_1, rgb_1)
        aca2_out = self.aca2(rgb_2, aca1_out)
        aca3_out = self.aca3(rgb_3, aca2_out)
        aca4_out = self.aca4(rgb_4, aca3_out)
